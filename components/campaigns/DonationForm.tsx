  ...
  return (
    <div className="glass-card p-6 space-y-5">
      <h3 className="font-bold text-base">
        {isActive ? "Fund This Grant Pool" : isExpired ? "Claim Refund" : "Grant Details"}
      </h3>
      ...
      {isActive && (
        <>
          {/* Quick amounts */}
          <div>
            <p className="text-xs text-muted-foreground mb-3">Quick select contribution amount (XLM)</p>
          ...
          {isConnected ? (
            ...
            >
              {isDonating ? (
                ...
                <>
                  <Heart className="w-4 h-4 relative z-10 animate-pulse text-red-300" />
                  <span>Contribute {amount > 0 ? formatXlm(amount) : ""}</span>
                </>
              )}
            </button>
          ) : (
      ...
      {isExpired && (
        <div className="space-y-4">
          <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
            ...
            <div>
              <p className="text-sm font-medium text-amber-400">Grant Closed</p>
              <p className="text-xs text-amber-400/70 mt-0.5">
                This grant program closed without hitting its target. Contributors can claim a full refund.
              </p>
            </div>
          </div>
        ...
        <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
          ...
          <div>
            <p className="text-sm font-medium text-emerald-400">
              {campaign.status === "Withdrawn" ? "Grant Claims Processed" : "Grant Fully Funded!"}
            </p>
            <p className="text-xs text-emerald-400/70 mt-0.5">
              {campaign.status === "Withdrawn"
                ? "The grant publisher has successfully claimed all funding."
                : "This grant program has met its allocation goal. Publisher can now claim funds."}
            </p>
          </div>
        </div>
      )}
