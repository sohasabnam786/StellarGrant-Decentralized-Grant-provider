...
async function main() {
  ...
  console.log("\n--- Deploying Grant Reward Token Contract ---");
  ...
  console.log("\n--- Deploying StellarGrant Contract ---");
  ...
  // 3. Initialize StellarGrant Contract (sets deployer as admin)
  console.log("\n--- Initializing StellarGrant Contract ---");
  ...
  // 4. Initialize Reward Token Contract (sets StellarGrant contract ID as admin, metadata)
  ...
  await invokeContract(
    ...
    [
      ...
      nativeToScVal("StellarGrant Reward"),
      nativeToScVal("GRNT"),
    ]
  );

  // 5. Set Reward Token ID in StellarGrant Contract
  console.log("\n--- Connecting Reward Token to StellarGrant ---");
  await invokeContract(
    rpc,
    deployerKeypair,
    crowdfundId,
  ...
  readmeContent = readmeContent.replace(
    /\| \*\*Contract ID\*\* \| .*\|/,
    `| **StellarGrant Contract ID** | \`${crowdfundId}\` |\n| **Grant Reward Token Contract ID** | \`${rewardTokenId}\` |`
  );
  ...
  console.log(`\n${"═".repeat(60)}`);
  console.log(`STELLARGRANT CONTRACT ID: ${crowdfundId}`);
  console.log(`GRANT REWARD TOKEN ID   : ${rewardTokenId}`);
