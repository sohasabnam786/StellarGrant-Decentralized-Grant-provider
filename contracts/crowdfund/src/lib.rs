...
use error::GrantError;

pub use error::GrantError as Error;

// ──────────────────────────────────────────────────────────────────────────────
// Inter-Contract Interface: Grant Token (previously Reward Token)
// ──────────────────────────────────────────────────────────────────────────────

/// Client interface for the Grant Reward Token contract.
/// This enables cross-contract calls from StellarGrantContract → GrantTokenContract.
...
#[contract]
pub struct StellarGrantContract;

#[contractimpl]
impl StellarGrantContract {
    // ── Admin ─────────────────────────────────────────────────────────────────

    /// Initialize the contract with an admin address.
    pub fn initialize(env: Env, admin: Address) -> Result<(), GrantError> {
    ...
    ///
    /// After this is set, every successful grant contribution will automatically trigger
    /// a cross-contract `mint` call on the reward token contract, crediting
    /// the contributor with 1 GRNT token per 1 stroop funded (1:1 ratio).
    ...
    ///
    /// # Arguments
    /// * `creator`     - Account creating the grant (must sign)
    /// * `title`       - Grant title (max 100 chars)
    /// * `description` - Short description (max 500 chars)
    /// * `goal`        - Funding goal/pool in stroops (1 XLM = 10_000_000 stroops)
    /// * `deadline`    - Unix timestamp (seconds) for the grant program deadline
    pub fn create_campaign(
        ...
        if goal <= 0 {
            return Err(GrantError::InvalidGoal);
        }

        let now = env.ledger().timestamp();
        if deadline <= now {
            return Err(GrantError::InvalidDeadline);
        }
        ...
        // Initialize applications/contributions list
    ...
    /// Fund/contribute XLM to a grant program.
    ///
    /// The funder must have pre-approved the transfer via the token contract.
    /// We use the native XLM token (Stellar Asset Contract).
    ///
    /// If a reward token is configured via `set_reward_token`, the contributor will
    /// automatically receive GRNT reward tokens equal to the stroops funded
    ...
    pub fn donate(
        ...
        // Transfer XLM from funder to contract
        ...
        // Record contribution/application detail
        ...
        // Mint GRNT reward tokens to the contributor.
    ...
    /// Claim/withdraw funds from a successful grant.
    ///
    /// Only the grant creator can claim after the goal is reached.
    pub fn withdraw(
        env: Env,
        campaign_id: u32,
        creator: Address,
    ) -> Result<i128, GrantError> {
        creator.require_auth();
        ...
        if grant.status != GrantStatus::Successful {
            ...
            return Err(GrantError::GrantNotSuccessful);
        }

        let amount = grant.raised;

        if amount == 0 {
            return Err(GrantError::NothingToWithdraw);
        }
    ...
    /// Refund a funder/applicant if the grant program expired without meeting its goal.
    pub fn refund(
        env: Env,
        campaign_id: u32,
        donor: Address,
        ...
        if grant.status != GrantStatus::Expired {
            return Err(GrantError::GrantNotExpired);
        }

        // Find the contributor's total contributions
        ...
        if refund_amount == 0 {
            return Err(GrantError::NoDonationFound);
        }
        ...
        // Update grant and application records
    ...
    pub fn get_campaign(env: Env, campaign_id: u32) -> Result<Grant, GrantError> {
        env.storage()
            .persistent()
            .get(&DataKey::Grant(campaign_id))
            .ok_or(GrantError::GrantNotFound)
    ...
    /// Get all contributions/applications for a grant.
    ...
    pub fn get_admin(env: Env) -> Result<Address, GrantError> {
        env.storage()
            .instance()
            .get(&DataKey::Admin)
            .ok_or(GrantError::NotInitialized)
...
mod test {
    ...
    fn setup_contract(env: &Env) -> (StellarGrantContractClient<'_>, Address) {
        let contract_id = env.register(StellarGrantContract, ());
        let client = StellarGrantContractClient::new(env, &contract_id);
    ...
    fn test_create_campaign() {
        ...
        let description = String::from_str(&env, "Funding for climate research");
    ...
    fn test_get_campaigns_paginated() {
        ...
        for i in 1u32..=3 {
            let title = String::from_str(&env, "Grant Program");
